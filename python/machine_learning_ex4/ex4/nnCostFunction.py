...
def nnGradient(nn_params, input_layer_size, hidden_layer_size,num_labels, X, y, lam):
	...
	row_index = np.arange(m)
	y_mat = np.zeros((m, num_labels))
	y_mat[row_index, y-1] = 1	

	# calc the gradient
	# shape is 5000x10
	delta_l_3 = a_3 - y_mat
	# shape is 10x26=10x5000 * 5000x26
	Theta2_grad = delta_l_3.T.dot(a_2) / m + lam / m * np.c_[np.zeros((num_labels, 1)), Theta2[:, 1:]]
	
	# shape is 5000x26=5000x10 * 10X26
	delta_l_2 = delta_l_3.dot(Theta2) * (a_2 * (1 - a_2))
	# shape is 25x401=25x5000 * 5000x401 
	Theta1_grad = delta_l_2[:, 1:].T.dot(X) / m + lam / m * np.c_[np.zeros((hidden_layer_size, 1)), Theta1[:, 1:]]
	
	grad = np.r_[Theta1_grad.flatten(order='F'), Theta2_grad.flatten(order='F')]
